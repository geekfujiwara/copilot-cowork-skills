...
正常系（SKILL.md）で詰まったときの検知と対処をここに集約する。多くは利用環境の
検証スクリプト（`${COWORK_BUILTIN_SKILLS_ROOT}/skills/scripts/`）で
...
- **対処**:
  ...
  - 利用環境で予約されている名前は避ける。上書きせず、衝突しない別名を使用する。
...
- **対処**: 古いスキルを利用環境の確認付き Delete 操作で削除してから再実行する。
...
- **対処**: ほぼ全て同じ — 広い方のスキルに
  ...
  RESOLVED として扱う。本スキル自身も「検証・採点・管理は利用環境の管理機能へ委譲」と
  明記してコエグジスタンスを回避している。
...
- **対処**: 別名にするか、上書き意図が明確なら `--force` を付けて再実行。既存スキルの
  「改善」が目的なら scaffold ではなく利用環境の最適化機能を使う。
